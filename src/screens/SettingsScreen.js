    ...
    <ScrollView style={st.container} contentContainerStyle={st.content}>
      ...
      <View style={st.section}>
        <Row label="Push alerts" sub="Notify when surges detected nearby"
          right={<Switch value={pushEnabled} onValueChange={setPush} trackColor={{ true:colors.primary }} thumbColor="#fff" />} />
        <Row label="Quiet hours" sub="Silence alerts 10pm – 7am"
          right={<Switch value={quietHours} onValueChange={setQuietHours} trackColor={{ true:colors.primary }} thumbColor="#fff" />} />
      </View>
        ...
        <View style={{ flexDirection:'row', flexWrap:'wrap', gap:spacing.sm }}>
          {RADIUS_OPTIONS.map(r => (
            <TouchableOpacity
              ...
              onPress={() => setRadius(r)}
            >
              <Text style={[st.chipText, alertRadius === r && { color:colors.primaryDark }]}>
      ...
      <View style={st.section}>
        <Row label="Social media signals" sub="Spike detection in your area"
          right={<Switch value={socialEnabled} onValueChange={setSocial} trackColor={{ true:colors.primary }} thumbColor="#fff" />} />
        <Row label="Community reports" sub="Reports from nearby users"
          right={<Switch value={reportsEnabled} onValueChange={setReports} trackColor={{ true:colors.primary }} thumbColor="#fff" />} />
      </View>

      <TouchableOpacity style={st.saveBtn} onPress={save}>
        <Text style={st.saveBtnText}>Save Preferences</Text>
      </TouchableOpacity>
      <Text style={[typography.micro, { textAlign:'center', marginTop:spacing.xl }]}>SurgeAlert v1.0.0</Text>
    </ScrollView>
  );
}
...
const st = StyleSheet.create({
  ...
  section:   { backgroundColor:colors.bg, borderRadius:12, borderWidth:1, borderColor:colors.border, overflow:'hidden', marginBottom:spacing.sm },
  ...
  saveBtnText:{ ...typography.body, color:colors.bg, fontWeight:'700' },
});
